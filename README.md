...
heap allocated objects. However, for types that represent primitive
values such as `Int` and `Double` which are allocated on the stack,
the compiler needs to perform an implicit type conversion whenever a
value of the parameter type is passed to a method of the generic class
or retrieved from it. For instance, when we call `q.enqueue(x)` on a
queue `q` of type `Queue[Int]` and an argument `x` of type `Int`, then
the generic byte code generated for `enqueue` expects a 64-bit pointer
to a heap allocated object as argument. However, `x` stores an `Int`
value, which only takes 32 bits.
...
conversions are opaque to the programmer from the perspective of the
program semantics, they incur a constant time and space overhead per
conversion that you should be aware of.
...
type of a public method). An example is the return type `T` of the
method `Cell.read`. If on the other hand, the type annotation
describes values that flow from the client to an instance of the
class (e.g., a parameter type of a public method), then this
annotation is contravariant. An example is the type `T` of the
parameter `x` of method `Cell.write`.

You may also think of covariant annotations as describing values that
are read by the client from the instance and contravariant annotations
...
`T` must be invariant in its surrounding generic class. 
...
The type constructor `List` occurs in the type annotations of the 
private `val` field of `Queue[T]`. Since `val` fields can only be
...
```scala
val q: Queue[AnyRef] = Queue.silly // OK because Queue.silly returns a
...
`AnyRef` instance. However, `AnyRef` instances do not have such a field.
...
class Queue[+T] ( ... ) {
 ...
 def enqueue[U](x: U): Queue[?] = new Queue[?](x :: queue)
}
```
...
supertype of `T` for otherwise the list `x :: queue`
...
class Queue[+T] ( ... ) {
 ...
 def enqueue[U >: T](x: U): Queue[U] = new Queue[U](x :: queue)
}
```
